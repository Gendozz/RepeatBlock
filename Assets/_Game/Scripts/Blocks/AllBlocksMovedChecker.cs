...
public class AllBlocksMovedChecker : IInitializable
{
    ...
    public void AddBlockAsChecked()
    {
        _blocksMovedAmount++;
        //Debug.Log($"Fired blocks {_blocksMovedAmount} of {_pathGenerator.WaypointPositions.Length} (_pathGenerator.WaypointPositions.Length). This hashcode is {GetHashCode()} ");

        if (_blocksMovedAmount == _pathGenerator.WaypointPositions.Length)
        {
            //await UniTask.Delay(TimeSpan.FromSeconds(0.2));
            //_signalBus.Fire<AllBlocksMoved>();
            //Debug.Log("All blocks moved fired");
            _blocksMovedAmount = 0;
        }
    }
